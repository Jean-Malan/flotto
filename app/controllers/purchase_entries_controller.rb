class PurchaseEntriesController < ApplicationController
  ...
  def index
    @purchase_entries = PurchaseEntry.all.where("user_id =?", current_user.id)
  end
  ...
  private
    ...
    def purchase_entry_params
      params.require(:purchase_entry).permit(:product_id, :description, :account_id, :quantity, :price, :total_price, :purchase_id, :user_id, :gl_account_id, :vat_type, :vat_amount, :net_price)
    end
end
